...
def _json_safe(value: Any) -> Any:
    """A BLOB column can hold raw bytes (e.g. the incorrect_attribute_datatype
    corruption). Represent as hex so the profile stays JSON-serializable and
    the value still reads as evidence of a datatype problem."""
    ...
    try:
        ...
        for table in tables:
            ...
            for col in cols:
                ...
                # known_values_from_samples runs on the raw samples so a BLOB
                # value (isinstance check fails) is correctly excluded from
                # the vocabulary rather than leaking in as a fake category.
                vocabulary = known_values_from_samples(samples)
                if vocabulary is not None:
                    profile["attribute_known_values"][key] = vocabulary
                profile["attribute_samples"][key] = [_json_safe(v) for v in samples]
