...
Injectors are grouped by OCEL dimension:
    ...
    * `src/order_management_corruption/object_issues.py`   — object-side: missing_object_type,
                                            missing_attribute_value,
                                            incorrect_object_type,
                                            incorrect_attribute_datatype,
                                            duplicate_objects_on_ids,
                                            duplicate_objects_on_attributes.
    * `src/order_management_corruption/event_issues.py`    — event-side: missing_event,
                                            missing_event_type,
                                            missing_event_timestamp.
    * `src/order_management_corruption/relation_issues.py` — relation-borne:
...
file to the destination and runs the selected `level`:
    ...
    from src.order_management_corruption import corrupt_database, DEFAULT_CLEAN_PATH, DEFAULT_FULL_PATH
...
from src.order_management_corruption.stages import corrupt_database
